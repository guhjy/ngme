...
$$
\begin{aligned}
Y_{ij} = {\bf x}_{ij}^{\top} {\bf \alpha} + {\bf d}_{ij}^{\top} {\bf U}_i + Z_{ij},
\end{aligned}
$$
and
$$
...
stochastic gradient algorithm. Predictions based on either nowcasting, 
forecastin or 
smoothing distributions could be obtained using the generic function `predict`. 
`print`, `summary`, `plot`, `fitted`, `residuals`, `intervals` 
...
Covariance structure of $W_i(t)$ could be specified through 
integrated Random-Walk or Matern.
